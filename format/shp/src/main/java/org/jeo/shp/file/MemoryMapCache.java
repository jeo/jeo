...
class MemoryMapCache {
	...
	MappedByteBuffer map(FileChannel wrapped, URL url, MapMode mode, long position, long size) throws IOException {
		...
		File file = ShpFiles.urlToFile(url).getCanonicalFile();
		...
		if(buffer == null) {
			synchronized (this) {
				...
				if(buffer == null) {
					...
					LOGGER.debug("Mapping and caching " + file.getAbsolutePath());
				}
			}
		} else {
		        LOGGER.debug("Using cached map for " + file.getAbsolutePath());
		}
	...
	void cleanFileCache(URL url) {
	    try {
    	    final File rawFile = ShpFiles.urlToFile(url);
    	    if(rawFile == null) {
    	        // not a local file
    	        return;
    	    }
    	    ...
    	    for (MappingKey key : keys) {
                if(key.file.equals(file)) {
                    ...
                    LOGGER.debug("Removed mapping for " + file.getAbsolutePath());
                }
            }
	    } catch(Throwable t) {
	        LOGGER.warn("An error occurred while trying to clean the memory map cache", t);
	    }
	}
	...
	void clean() {
	    ...
	    for (MappingKey key : keys) {
            MappedByteBuffer buffer = buffers.remove(key);
            NIOUtilities.clean(buffer, true);
            LOGGER.debug("Removed mapping for " + key.file.getAbsolutePath());
        }
	}
